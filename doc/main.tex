...
\begin{itemize}
\item with an route which is already in the route box, then press \inlinefigure{figures/existing-route-button.png}. Then confirm and double click on the route with which you want to make the transport, or select it and press Enter. 
...
\subsection{Entering a position: The position frame}\label{section-position-frame}

\begin{figure}
  \centering
  \includegraphics[width=0.3\textwidth]{figures/position-frame.png}
  \caption{
...
In order to enter a position in \thel (for example, the last estimated position, or the position where something worthwhile happened), go on the main frame and click on \inlinefigure{figures/plus-button.png} at the bottom of the position frame. The position frame will appear, see \cref{fig-position-frame}. 
