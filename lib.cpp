...
void ChartFrame::GetCoastLineData_3D(void){
    ...
    for(x_3d.clear(), y_3d.clear(), i=0; i<data_3d.size(); i++){
        ...
        if((i % every) == 0){
            ...
            if(cos(euler_a -((data_3d[i]).lambda))*cos(((data_3d[i]).phi))*sin(euler_c) + cos(euler_c)*(cos(euler_b)*cos(((data_3d[i]).phi))*sin(euler_a -((data_3d[i]).lambda)) + sin(euler_b)*sin(((data_3d[i]).phi))) < 0.0){
