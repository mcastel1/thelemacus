...
public:
    ...
    wxSize size_chart;
    unsigned int  /*these are the values of width/height_chart when the chart is first drawn*/width_chart_0, height_chart_0, width_plot_area, height_plot_area, tick_length, /* gamma_lambda is the compression factor which allows from switching from increments in degrees to increments in arcminutes when setting the ticks on the x axis, and similarly for gamma_phi*/gamma_lambda, gamma_phi;
    ...
    void Render_3D(wxDC&);
    void PutLabel(const Position&, Angle, Angle, Int, String);
