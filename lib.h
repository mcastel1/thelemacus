...
public:
    ...
    bool operator == (const Angle&), operator == (const double&), operator > (const Angle&), operator < (const Angle&), operator > (const double&), operator < (const double&);
...
public:
    ...
    Angle rotation_angle, /*an angle containing the middle longitude of the current 3D projection, rounded up to the closest value which is a multiple of delta_lambda, used for drawing things in the middle of the projection*/lambda_middle, /*lambda/phi_start/end are the start/end values of longidue/latitude adapted in the right form ro the loopws which draw meridians/parallels*/ lambda_start, lambda_end, phi_start, phi_end;
