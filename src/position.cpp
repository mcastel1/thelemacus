...
bool Position::distance(Position p, Length* l, String name, [[maybe_unused]] String prefix) {
    ...
    l->set((Re.value) * acos(cos((lambda)-(p.lambda)) * cos(phi) * cos(p.phi) + sin(phi) * sin(p.phi)), LengthUnit_types[0]);
    ...
    if(check){
        ...
        Angle a;
        a.set(String(""), (l->value) / (Re.value), prefix);
...
bool Position::is_in(Route route, [[maybe_unused]] String prefix) {
    ...
    if ((route.type) == (Route_types[2])) {
        ...
        return(d <= ((Re.value) * (route.omega.value)));
