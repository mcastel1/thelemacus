                ...
                case 0: {
                    ...
                    if (inclusion((*(draw_panel->rectangle_observer)), true, &t, String("")) == 1) {
                        ...
                        if ((t[0] == 0.0) && (t[1] == 0.0)) {
                            ...
                            ((*s)[1]).set(2.0 * M_PI * (Re.value) * sin(omega), LengthUnit_types[0]);
                            ...
                            for (s->resize(t.size()), i = 0; i < (t.size()); i++) {
                                ...
                                ((*s)[i]).set(((t[i]).value) * (Re.value) * sin(omega), LengthUnit_types[0]);
                ...
                case 1: {
                    ...
                    if (inclusion((*(draw_panel->circle_observer)), true, &t, String("")) == 1) {
                        ...
                        if ((t[0] == 0.0) && (t[1] == 0.0)) {
                            ...
                            compute_end(
                                        Length(
                                               ((Angle(mean_value(t[0], t[1]))).value) * ((Re.value) * sin(omega))
                                               ),
                                        String(""));
                            ...
                            compute_end(
                                        Length(
                                               ((Angle(mean_value(t[0], t[1]) + M_PI)).value) * ((Re.value) * sin(omega))
                                               ),
                                        String(""));
                            ...
                            if (l2 > l1) {
                                ...
                                ((*s)[0]).set(((t[0]).value) * ((Re.value) * sin(omega)), LengthUnit_types[0]);
                                ((*s)[1]).set(((t[1]).value) * ((Re.value) * sin(omega)), LengthUnit_types[0]);
                                
                            }else{
                                
                                ((*s)[0]).set(((t[1]).value) * ((Re.value) * sin(omega)), LengthUnit_types[0]);
                                ((*s)[1]).set((2.0 * M_PI + ((t[0]).value)) * ((Re.value) * sin(omega)), LengthUnit_types[0]);
...
bool Route::closest_point_to(Position* p, Angle* tau, Position q, [[maybe_unused]] String prefix) {
    ...
    if (type == (Route_types[2])) {
        ...
        compute_end(Length((t_1.value) * (Re.value) * sin(omega)), new_prefix);
        p_1 = (*end);
        q.distance(p_1, &s_1, String("Distance with respect to p_1"), new_prefix);

        compute_end(Length((t_2.value) * (Re.value) * sin(omega)), new_prefix);
        p_2 = (*end);
        ...
        if (type == Route_types[0]) {
            ...
            if (type == Route_types[1]) {
                ...
                if (intersection(circle, true, t, new_prefix) == 0) {
                    ...
                    if (reference_position->is_in(circle, prefix)) {
                        ...
                        if (write_t) {
                            ...
                            ((*t)[1]).set(String(""), (length->value) / (Re.value), new_prefix);
                    ...
                    if (write_t) {
                        ...
                        case 1: {
                            ...
                            if (reference_position->is_in(circle, prefix)) {
                                ...
                                //this->reference position is not included into the circle of circle -> this->end must be included into the circle of circle -> the part of *this comprised into circle is the one with  (*t)[0] <= t <= (l.value)/(Re.value)

                                set_length_from_time_speed();
                                my_push_back(t, Angle(String(""), (length->value) / (Re.value), new_prefix));
                        ...
                        case 2: {
                            ...
                            compute_end(Length((Re.value) * (((((*t)[0]).value) + (((*t)[1]).value)) / 2.0)), String(""));
                ...
                if (type == Route_types[2]) {
                    ...
                    if (d < ((Re.value) * ((omega + (circle.omega)).value))) {
...
int Route::inclusion(PositionRectangle rectangle, bool write_t, vector<Angle>* t, [[maybe_unused]] String prefix) {
    ...
    if (type == (Route_types[0])) {
        ...
        if (type == (Route_types[1])) {
            set_length_from_time_speed();
            my_push_back(&u, Angle((length->value) / (Re.value)));
        }
        ...
        for (output = 0, is_fully_included = true, i = 0; i < (u.size()) - 1; i++) {
            ...
            if (type == (Route_types[1])) {
                compute_end(Length((Re.value) * (((u[i]).value) + ((u[i + 1]).value)) / 2.0), String(""));
            }
            if (type == (Route_types[2])) {
                compute_end(Length((Re.value) * sin(omega) * (((u[i]).value) + ((u[i + 1]).value)) / 2.0), String(""));
            }
...
int Route::intersection(Route route, bool write_t, vector<Angle>* t, [[maybe_unused]] String prefix) {
    ...
    if ((route.type) == (Route_types[2])) {
        ...
        if (type == (Route_types[1])) {
            ...
            d.set(String(""), (Re.value) * acos(cos_ts.value), prefix);
            ...
            d.set(String(""), (Re.value) * (M_PI - acos(cos_ts.value)), prefix);
            if (compute_end(d, prefix)) {
            ...
            //obtain the minimum distance across all cases, which may be 2, 3, or 4, and chekwhetehr it is smaller than (Re.value) * apertur angle of route
            if ((*min_element(s.begin(), s.end())) < (Re.value) * (route.omega.value)) {
                //in this case, *this and route intersect: I compute the values of the parametric angle t which parametrizes *this and at which the distance betweeen (point on *this at t) and (GP of route) is equal to (Re.value)*(angular aperture of route)
                ...
                //these are the values of cos(t) such that the distance between this->end at t  and route.reference_position equals (Re.value)*(route.omega), i.e., it is the value of cos(t) such that end(t) lies on route. There are two of them.
                ...
                if ((/*when I solve the equations a cos t + b * sqrt(1-(cos t)^2)  = - cos(route.omega), I manipulate the euqation and then square both sides, thus introducing spurious solutions. This condition allows me to check which one among the spurious solutions is valid. */-((a.value) * (cos_t_p.value) + cos(route.omega)) / (b.value) > 0.0) && compute_end(Length((Re.value) * acos(cos_t_p)), prefix)) {
                    ...
                    if (compute_end(Length((Re.value) * (2.0 * M_PI - acos(cos_t_p))), prefix)) {
                ...
                if ((/*when I solve the equations a cos t + b * sqrt(1-(cos t)^2)  = - cos(route.omega), I manipulate the euqation and then square both sides, thus introducing spurious solutions. This condition allows me to check which one among the spurious solutions is valid. */-((a.value) * (cos_t_m.value) + cos(route.omega)) / (b.value) > 0.0) && compute_end(Length((Re.value) * acos(cos_t_m)), prefix)) {
                    ...
                    if (compute_end(Length((Re.value) * (2.0 * M_PI - acos(cos_t_m))), prefix)) {
            ...
            if (type == (Route_types[2])) {
                ...
                if (/*this is the condition that *this and route intersect*/(d > (Re.value) * fabs((omega.value) - (route.omega.value))) && (d < (Re.value) * ((omega + (route.omega)).value))) {
...
int Route::crossing(Route route, vector<Position>* p, double* cos_crossing_angle, [[maybe_unused]] String prefix) {
    ...
    if (!((type == (Route_types[2])) && (route.type == (Route_types[2])))) {
        ...
        if ((abs((omega.value) - (route.omega.value)) < (theta.value)) && ((theta.value) < (omega.value) + (route.omega.value))) {
            ...
            compute_end(Length((Re.value) * sin(omega.value) * ((t[0]).value)), new_prefix);
            (*p)[0] = (*end);
            ((*p)[0]).label.set(String(""), String("crossing"), prefix);

            compute_end(Length((Re.value) * sin(omega.value) * ((t[1]).value)), new_prefix);
            (*p)[1] = (*end);
            ((*p)[1]).label.set(String(""), String("crossing"), prefix);

            route.compute_end(Length((Re.value) * sin(route.omega.value) * ((u[0]).value)), prefix);
            ...
            if (check) {
                ...
                compute_end(Length((Re.value) * sin(omega) * ((t[0]).value)), prefix);
    ...
    switch ( type.position_in_list(Route_types)) {
        ...
        case 0:{
            ...
            if(fabs(C) > epsilon_double){
                ...
                s = GSL_SIGN(cos(Z));
                length->set(s * 2.0*(Re.value)/sqrt(C) *( atan(eta) - atan( eta * exp(- s * sqrt(C/(1.0-C)) * t ) ) ),
                           LengthUnit_types[0]);
                ...
                length->set(2.0*(Re.value)*t*eta/(1.0+gsl_pow_2(eta)),
                           LengthUnit_types[0]);
    ...
    switch (type.position_in_list(Route_types)) {
        ...
        case 0:{
            ...
            if (((Z.value) != M_PI_2) && ((Z.value) != 3.0 * M_PI_2)) {
                ...
                * log(1.0 / eta * tan(-tau * sqrt(C) * (length->value) / (2.0 * (Re.value)) + atan(sqrt((1.0 - sin(reference_position->phi.value)) / (1.0 + sin(reference_position->phi.value))))));
                
            }
            else {
                //this is the limit of the expression above in the case Z -> pi/2
                
                (t.value) = (length->value) * (1.0 + gsl_pow_2(eta)) / (2.0 * (Re.value) * eta);
        ...
        case 1:{
            ...
            t.set(String(""), (length->value) / (Re.value), prefix);
        ...
        case 2:{
            ...
            set_length_from_time_speed();
            t.set(String(""), (length->value) / ((Re.value) * sin(omega)), prefix);
...
double Route::lambda_minus_pi(double t, void* route) {
    ...
    r->length->set((Re.value) * sin((r->omega.value)) * t, LengthUnit_types[0]);
...
void Route::lambda_min_max(Angle* lambda_min, Angle* lambda_max, [[maybe_unused]] String prefix) {
    ...
    if (type == (Route_types[2])) {
        ...
        if (abs(-tan(reference_position->phi.value) * tan((omega.value))) < 1.0) {
            ...
            length->set((Re.value) * sin((omega.value)) * (t_max.value), LengthUnit_types[0]);
            compute_end(new_prefix);
            p_max = (*end);
            
            //p_min =  Position on circle of equal altitude  at t = t_min
            length->set((Re.value) * sin((omega.value)) * (t_min.value), LengthUnit_types[0]);
            compute_end(new_prefix);
            p_min = (*end);
...
bool Route::phi_min_max(Angle* phi_min, Angle* phi_max, [[maybe_unused]] String prefix) {
    ...
    switch (type.position_in_list(Route_types)) {
        ...
        case 1:{
            ...
            if((0.0 <= (Re.value)*ts) && ((*length) >= (Re.value)*ts)){
                
                //                t.push_back(Angle(ts));
                
                compute_end(Length((Re.value)*ts), String(""));
                my_push_back(&phi, end->phi.normalize_pm_pi_ret());
                
            }
            
            //there are two potential stationary points for the latitude vs t: include in phi the second one, if it lies on *this
            if((0.0 <= (Re.value)*(ts+M_PI)) && ((*length) >= (Re.value)*(ts+M_PI))){
                                
                compute_end(Length((Re.value)*(ts+M_PI)), String(""));
                my_push_back(&phi, end->phi.normalize_pm_pi_ret());
        ...
        case 2:{
            ...
            length->set((Re.value) * sin((omega.value)) * 0.0, LengthUnit_types[0]);
            compute_end(new_prefix);
            p_max = (*end);
            
            length->set((Re.value) * sin((omega.value)) * M_PI, LengthUnit_types[0]);
            compute_end(new_prefix);
            p_min = (*end);
