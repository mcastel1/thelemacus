...
void SightFrame::OnPressReduce(wxCommandEvent& event) {
    ...
    if (position_in_listcontrol_sights == -1) {
        ...
        if ((sight->related_route.get()) != -1) {
            //sight has a related Route -> reduce sight and write the resulting Route into such related Route

            sight->reduce(&((parent->data->route_list)[sight->related_route.get()]), String(""));
            ...
            sight->related_route.set(((int)(parent->data->route_list.size())) - 1);
            ((parent->data->route_list)[(parent->data->route_list.size()) - 1]).related_sight.set(((int)sight_position));
    ...
    parent->highlight_route->set_value(sight->related_route.get());
    ...
    AnimateToObject<Route, HighlightObject<ListFrame, UnsetIdling<ListFrame>> > animate(parent, &((parent->data->route_list)[sight->related_route.get()]), parent->highlight_route_and_unset_idling);
    animate.operator()();
